    ...
    module FormOptionsHelper
      def us_state_options_for_select(selected = nil, us_state_options = {})
        ...
        states_label = case us_state_options[:show]
          when :full_abb          then lambda { |state| [state[0], state[1]] }
          when :full              then lambda { |state| [state[0], state[0]] }
          when :abbreviations     then lambda { |state| [state[1], state[1]] }
          when :abb_full_abb      then lambda { |state| ["#{state[1]} - #{state[0]}", state[1]] }
